...
export class OcrProcessor {
  ...
  async processFile(inputKey: string, outputKey: string): Promise<OcrResult> {
    try {
      ...
      if (!response.Body) {
        throw new OcrError(ErrorType.FILE_NOT_FOUND, 'File not found in S3');
      }
    ...
    try {
      ...
      if (engineType === 'textract-analyze') {
        ...
        throw new OcrError(
          ...
          'Unsupported OCR engine. Use textract or textract-analyze.'
        );
      }
    } finally {
      ...
      if (tableBlock.Relationships) {
        ...
        if (cellRelationship?.Ids) {
          ...
          if (mappedCells.length > 0) {
            const maxRow = Math.max(...mappedCells.map(c => c.rowIndex || 0));
            const maxCol = Math.max(...mappedCells.map(c => c.columnIndex || 0));
          ...
          tables.push({ cells: mappedCells, rows });
        }
      }
    }
